...
#Updated by Shuzhan Fan on 05/15/2018
...
def search_latest(path, row):
	...
	path_str = 'gs://gcp-public-data-landsat/LC08/01/' + path + '/' + row
	...
	if stdout:
		...
		data_path_latest = data_path_list[-1]           #e.g. gs://gcp-public-data-landsat/LC08/01/022/039/LC08_L1TP_022039_20180506_20180506_01_T1/
		data_latest = data_path_latest.split('/')[-2]   #e.g. LC08_L1TP_022039_20180506_20180506_01_T1
		date_str = data_latest.split('_')[3]            #e.g. 20180506
...
def check_date(path, row, dobj):
	...
	if stdout:
		...
		data_str = 'LC08_L1TP_' + path + row + '_' + date_str
...
def download(path, row, date):
	...
	p = Popen('gsutil -m cp -r '+data_path_str+' ./data', stdout=PIPE, stderr=PIPE, shell=True)
	stdout = p.stdout.read()
	stderr = p.stderr.read()
	if stderr:
		logger.error(stderr)
